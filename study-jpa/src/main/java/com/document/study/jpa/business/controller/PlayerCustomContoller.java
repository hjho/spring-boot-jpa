...
public class PlayerCustomContoller {
	...
	@GetMapping("/query")
	public ResponseEntity<List<PlayerRVO>> query() {
		return new ResponseEntity<List<PlayerRVO>>(playerCustomService.query(), CODE_200);
	}
	@GetMapping("/query/{code}")
	public ResponseEntity<PlayerRVO> queryOne(@PathVariable String code) {
		return new ResponseEntity<PlayerRVO>(playerCustomService.queryOne(code), CODE_200);
	}
